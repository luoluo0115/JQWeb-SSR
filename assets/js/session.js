export const qhsession = (function () {
  'use strict'
  var VERSION = '1.0.0'
  // 支持的数据类型
  ...
  if (!sessionStorage.getItem(KEYWORDID)) {
    qhcommon.session.set(KEYWORDID, { KEYWORDID: 'Object' })
  }
  if (!localStorage.getItem(KEYWORDID)) {
    qhcommon.local.set(KEYWORDID, { KEYWORDID: 'Object' })
  }
  // 是否存在
  function isexist (key, type) {
    if (type === 'session') {
      ...
      var flag = !!((key && localStorage.getItem(key)))
      return flag
    }
  }
  ...
  function updateKeys (key, keytype, operate, saveType) {
    var keys = saveType === 'session' ? qhcommon.session.get(KEYWORDID) : qhcommon.local.get(KEYWORDID)
    ...
    saveType === 'session' ? qhcommon.session.set(KEYWORDID, keys)
      : qhcommon.local.set(KEYWORDID, keys)
  }
  // 对外提供接口
  return qhcommon
}())
